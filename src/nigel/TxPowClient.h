...
/* Client for an external transaction PoW server (DeroGold-txpow-server).

   Nothing is configured by default, so every wallet computes its proof of
   work locally exactly as before. A wallet that lets the user point at a
...
namespace TxPowClient
{
    ...
    /* A solver bound to the current settings, or an empty function when no
       server is configured. Pass straight to generateTransactionPoW(). */
