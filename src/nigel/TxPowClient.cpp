...
namespace
{
    ...
    /* The origin plus the mount prefix, for logs. */
        ...
        std::array<uint8_t, CryptoNote::TX_POW_NONCE_SIZE> &nonce)
    {
        ...
        if (!j.is_object())
        {
            logRemote("reply is not a JSON object (HTTP " + std::to_string(reply.status) + ")", Logger::WARNING);
            jobId.clear();
            return false;
        }
        ...
        {
            logRemote(
                "refused the job (HTTP " + std::to_string(reply.status)
                    + "): " + j.value("error", std::string("no reason given")),
                Logger::WARNING);
            jobId.clear();
            return false;
        }

        if (hasKey(j, "job_id") && j["job_id"].is_string())
        {
...
namespace TxPowClient
{
    ...
    std::string probe(const std::string &hostIn, const uint16_t port, const bool ssl)
    {
        ...
        try
        {
            ...
            if (!health.is_object() || health.value("status", std::string()) != "OK" || !hasKey(health, "threads"))
            {
