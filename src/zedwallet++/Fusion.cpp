...
bool optimizeRound(const std::shared_ptr<WalletBackend> walletBackend)
{
    ...
    if (std::get<2>(walletBackend->getSyncStatus()) > CryptoNote::parameters::FUSION_FEE_V1_HEIGHT && std::get<2>(walletBackend->getSyncStatus()) < CryptoNote::parameters::FUSION_FEE_V1_EXIT_HEIGHT)
    {
