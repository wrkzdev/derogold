...
namespace
{
    ...
    struct BinaryVariantTagGetter : boost::static_visitor<uint8_t>
    {
        ...
        uint8_t operator()(const CryptoNote::KeyInput)
        {
            return 0x2;
        }

        uint8_t operator()(const CryptoNote::KeyOutput)
        {
            return 0x2;
        }

        uint8_t operator()(const CryptoNote::Transaction)
        {
            return 0xcc;
        }
    ...
    template<typename T> bool serializePod(T &v, Common::StringView name, CryptoNote::ISerializer &serializer)
    {
