...
namespace CryptoNote
{
    ...
    static_assert(
        CryptoNote::UpgradeDetectorBase::UNDEF_HEIGHT == UINT32_C(0xFFFFFFFF),
        "UpgradeDetectorBase::UNDEF_HEIGHT has invalid value");

    template<typename BC> class BasicUpgradeDetector : public UpgradeDetectorBase
    {
      public:
        ...
        bool init()
        {
            ...
            else if (!m_blockchain.empty())
            {
                if (m_blockchain.size() <= upgradeHeight + 1)
                {
                    ...
                    {
                        logger(Logging::ERROR, Logging::BRIGHT_RED)
                            << "Internal error: block at height " << (m_blockchain.size() - 1)
                ...
                else
                {
                    ...
                    {
                        logger(Logging::ERROR, Logging::BRIGHT_RED)
                            << "Internal error: block at height " << upgradeHeight << " has invalid version "
                    ...
                    {
                        logger(Logging::ERROR, Logging::BRIGHT_RED)
                            << "Internal error: block at height " << (upgradeHeight + 1) << " has invalid version "
        ...
        size_t getNumberOfVotes(uint32_t height)
        {
            ...
            for (size_t i = height + 1 - m_currency.upgradeVotingWindow(); i <= height; ++i)
            {
                ...
                voteCounter +=
                    (b.majorVersion == m_targetVersion - 1) && (b.minorVersion == BLOCK_MINOR_VERSION_1) ? 1 : 0;
            }
      ...
      private:
        uint32_t findVotingCompleteHeight(uint32_t probableUpgradeHeight)
        {
            ...
            uint32_t probableVotingCompleteHeight = probableUpgradeHeight > m_currency.maxUpgradeDistance()
