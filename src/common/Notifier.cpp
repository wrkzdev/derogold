...
#include <spawn.h>
#define DEROGOLD_NOTIFIER_HAVE_POSIX_SPAWN 1
#if defined(__APPLE__)
#include <crt_externs.h>
#define DEROGOLD_NOTIFIER_ENVIRON (*_NSGetEnviron())
#else
extern char **environ;
#define DEROGOLD_NOTIFIER_ENVIRON environ
#endif
#endif
#endif
...
#if defined(DEROGOLD_NOTIFIER_HAVE_POSIX_SPAWN)
        /* posix_spawn avoids duplicating the (potentially multi-GB) daemon
           address space that a plain fork() would touch. */
        const int spawnError = posix_spawnp(&pid, args[0], nullptr, nullptr, args.data(), DEROGOLD_NOTIFIER_ENVIRON);
        if (spawnError != 0)
        {
