...
/* set_ca_cert_path() only exists on httplib::Client when cpp-httplib is built
   with OpenSSL. Without it there is no TLS to point at a trust store and no
   such member to call, so the helper has to compile away there. */
#if defined(__ANDROID__) && defined(CPPHTTPLIB_OPENSSL_SUPPORT)
#define DEROGOLD_HAVE_ANDROID_CA_CERT_DIR 1
#include <sys/stat.h>
#endif
...
namespace Common
{
    ...
    {
#if defined(DEROGOLD_HAVE_ANDROID_CA_CERT_DIR)
