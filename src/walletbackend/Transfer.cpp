...
namespace SendTransaction
{
        ...
        const std::vector<uint8_t> extraData)
    {
         ...
         * the sigs. */
        /* When a wallet has configured a Tx PoW server the solver asks it
           first and falls back to this CPU; otherwise it is empty and the
           work is done here as always. */
        setupTX.extra = CryptoNote::generateTransactionPoW(setupTX, extra, TxPowClient::solver());
