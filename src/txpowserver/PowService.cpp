...
namespace
{
        ...
        bool &tooDifficult)
    {
        ...
        /* Equal sums are a fusion transaction, which DeroGold holds to the
           same proof of work as any other, so only a negative fee is refused. */
        ...
        if (extra.size() < 1 + CryptoNote::TX_POW_NONCE_SIZE
            || extra[extra.size() - 1 - CryptoNote::TX_POW_NONCE_SIZE]
                   != Constants::TX_EXTRA_TRANSACTION_POW_NONCE_IDENTIFIER)
        {
        ...
        job.difficulty =
            CryptoNote::transactionPoWDifficulty(height.value_or(std::numeric_limits<uint64_t>::max()));
...
void PowService::solve(const std::shared_ptr<PowJob> &job)
{
    ...
    auto worker = [&, this](const uint64_t startNonce) {
               ...
               && !m_stop.load(std::memory_order_relaxed))
        {
            ...
            Crypto::Hash hash;
            CryptoNote::transactionPoWHash(prefix, hash);
...
void PowService::housekeeping()
{
    ...
    for (auto it = m_jobs.begin(); it != m_jobs.end();)
    {
        ...
        if (job->finished()
            && msBetween(job->finishedAt, now) > static_cast<uint64_t>(m_limits.resultTtlSeconds) * 1000)
        {
            it = m_jobs.erase(it);
        }
        else
