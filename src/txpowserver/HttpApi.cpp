...
namespace
{
    ...
    /* nlohmann-json here is 3.2.0, which predates json::contains(). */
...
void HttpApi::configure(httplib::Server &server)
{
    /* Long polls hold a handler thread each, so size the pool for the number
       of jobs that can be waiting rather than for the CPU count. The vendored
       cpp-httplib's pool is fixed-size, so leave headroom for the polls and
       /stats requests arriving alongside them. */
    const size_t poolThreads = std::max<size_t>(8, m_config.maxQueue) * 2;

    server.new_task_queue = [poolThreads] { return new httplib::ThreadPool(poolThreads); };
...
uint32_t HttpApi::waitMs(const httplib::Request &req, const nlohmann::json *body) const
{
    ...
    if (body != nullptr && hasKey(*body, "wait_ms") && (*body)["wait_ms"].is_number_unsigned())
    {
...
void HttpApi::handleSubmit(const httplib::Request &req, httplib::Response &res)
{
    ...
    if (!hasKey(body, "prefix") || !body["prefix"].is_string())
    {
    ...
    if (hasKey(body, "height") && body["height"].is_number_unsigned())
    {
