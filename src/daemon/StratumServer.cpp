...
namespace Daemon
{
    namespace
    {
        ...
        /* nlohmann::json::contains() arrived in 3.6.0 and the vendored copy
           here is 3.2.0, so ask the iterator instead. */
        bool hasKey(const nlohmann::json &object, const std::string &key)
        {
        ...
        {
            if (!hasKey(object, key) || !object.at(key).is_string())
            {
                return std::string();
            }
    ...
    bool StratumServer::start()
    {
        ...
        try
        {
            m_listener = System::TcpListener(m_dispatcher, System::Ipv4Address(m_bindAddress), m_port);
        }
    ...
    void StratumServer::acceptLoop()
    {
        while (m_running)
        {
            try
            {
                ...
                try
                {
                    client->peer = client->connection.getPeerAddressAndPort().first.toDottedDecimal();
                }
    ...
    void StratumServer::handleLogin(const ClientPtr &client, const nlohmann::json &request)
    {
        const nlohmann::json params =
            hasKey(request, "params") && request.at("params").is_object() ? request.at("params") : nlohmann::json::object();
        ...
        const nlohmann::json &result)
    {
        ...
        response["id"] = hasKey(request, "id") ? request.at("id") : nlohmann::json(nullptr);
