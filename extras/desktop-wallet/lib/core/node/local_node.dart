...
/// currently doing.
///
/// The node is a child process, not a library. `DeroGoldd`'s `main()` calls
...
///
/// It is unauthenticated: the wallet backend has no way to send an access
/// token (Nigel builds its request headers itself and only ever sets a
/// User-Agent). The daemon can also expose an AF_UNIX socket with 0600
/// permissions on POSIX, which would be tighter, but Windows compiles that out
/// entirely, so loopback is what both ends can always agree on.
const String kLocalNodeHost = '127.0.0.1';

/// Config filename inside the node data directory.
const String _kConfigFile = 'derogold-node.json';
...
const String _kPidFile = 'derogold-node.pid';

/// The daemon's own log, which is where a startup failure explains itself.
const String kLocalNodeLogFile = 'derogoldd.log';
...
class LocalNodeConfig {
  ...
  ///
  /// `DEROGOLD_DAEMON_EXTRA_ARGS` is appended last, space separated. It exists
  /// for daemons built without the options this list assumes — a RocksDB
  /// linked without ZSTD needs `--db-enable-compression=false` or it exits at
...
class LocalNodeState {
  ...
  /// The lite height the daemon itself reports, from `/info`'s
  /// `lite_node_height`. Zero until the RPC answers, and it should equal the
  /// configured height — a difference means the running daemon is not the one
...
class LocalNodePaths {
  ...
  static String get daemonName =>
      Platform.isWindows ? 'DeroGoldd.exe' : 'DeroGoldd';

  /// Finds the daemon to run, or null if it is not shipped alongside the app.
  ///
  /// `DEROGOLD_DAEMON_PATH` wins so a development build can point at a build
  /// tree without copying anything.
  static File? findDaemon() {
    final override = Platform.environment['DEROGOLD_DAEMON_PATH'];
...
Future<bool> _isWritable(String path) async {
  final probe = File(p.join(path, '.derogold-write-probe'));
  try {
...
/// Extra daemon flags from `DEROGOLD_DAEMON_EXTRA_ARGS`, split on whitespace.
///
...
List<String> extraDaemonArguments() {
  final raw = Platform.environment['DEROGOLD_DAEMON_EXTRA_ARGS'];
