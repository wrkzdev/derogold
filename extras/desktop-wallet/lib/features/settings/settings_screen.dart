...
const _kLastWalletKey = 'derogold_last_wallet_path';
...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  Future<void> _resetScanHeight() async {
    final liteStart =
        ref.read(statusProvider).valueOrNull?.pruneFloor ?? 0;

    final heightCtrl =
        TextEditingController(text: liteStart > 0 ? '$liteStart' : '0');
    // A rescan from far down the chain runs for hours. Asked for once, with
    // the cost stated, rather than discovered by watching the wallet sit at
    // 3% for an afternoon.
                ...
                children: [
                  ...
                  if (slow && !tooLow) ...[
                    ...
                    Text(
                      ...
                      'chain. That can take a very long time — many hours, or '
                      'days against a busy node. If you know roughly when this '
                      'wallet was first used, rescan from that height instead.',
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(tr?.incomingTxAlertsSubtitle ?? 'Show a desktop notification when DEGO is received', style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 12)),
                          ],
                        ),
                      ),
                      Switch(
