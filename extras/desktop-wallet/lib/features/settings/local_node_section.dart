...
class LocalNodeSection extends ConsumerWidget {
  ...
  /// keep it at or below the height this wallet was created at — are stated
  /// before the field.
                    ...
                    children: [
                      Text(
                        dlgTr?.localNodeSetupCost ??
                            'Before you start:\n'
                                '• Several GB of disk space, and the whole chain is downloaded once.\n'
                      ...
                      Text(
                        // The wallet's own start height is not something this
                        // build can read back, so the rule is stated rather
                        // than checked. Getting it wrong is not dangerous —
                        // the balance simply reads low — but it is permanent
                        // for this node.
                        'Blocks below this height are downloaded and checked, '
                        'then only the index later blocks need is kept. Keep '
                        'it at or below the height this wallet was created at, '
                        'or the node can never show its older transactions.',
                        style: const TextStyle(
                      ...
                      Text(
                        dlgTr?.localNodeDataFolderHelp ??
                            'Several GB is written here. Pick a drive with '
                                'room for it.',
                        style: const TextStyle(
                            color: kTextSecondary, fontSize: 12, height: 1.4),
                      ),
