...
This folder contains a static block explorer UI for DeroGold.

It is intended to be served by a normal web server such as nginx while the
DeroGold daemon runs separately with explorer RPC enabled.

## Requirements

- A built and running DeroGold daemon
...
```powershell
.\DeroGoldd.exe --daemon-mode explorer --enable-cors "*"
```

Notes:

- `--daemon-mode explorer` is required for the explorer-specific RPC methods
- `--enable-cors` takes a domain (`*` for any) and is only needed when the
  browser talks to the daemon directly
...
- `vendor/derogold-crypto.js`: self-contained address and key primitives used by the wallet tools
...
server {
    ...
    root /var/www/derogold-explorer;
    index index.html;
...
`vendor/derogold-crypto.js`, so shipping the markup without the vendor directory
...
<script src="vendor/derogold-crypto.js?v=20260830"></script>
...
the browser against `vendor/derogold-crypto.js`. None of them contacts the daemon,
...
**Short payment IDs cannot be looked up.** Where they appear at all they are
encrypted against the shared secret between sender and receiver, so the same
payment ID is different bytes in every transaction and there is nothing stable
to index. Searching for one routes to a page that explains this, rather than
reporting "not found" — which would wrongly suggest the payment ID had never
been used.

### Payment ID length

DeroGold has one integrated address form. `src/config/WalletConfig.h` derives
`integratedAddressLength` from a 64-character payment ID and defines no short
one, so:

- 64 hex characters produces a 185-character integrated address

The 16-character form that WrkzCoin and some other forks accept is not offered.
An address built that way would be rejected by this chain's wallets, so the
tools refuse the length rather than minting one.
...
`vendor/derogold-crypto.js` reimplements the daemon's primitives in plain JavaScript
...
CRC32 and ed25519 against published vectors, round-trips a real DeroGold
...
### Tools menu items show "derogold-crypto.js failed to load"

Confirm `vendor/derogold-crypto.js` was copied to the web root alongside
