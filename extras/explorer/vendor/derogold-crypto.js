...
/* ═══════════════════════════════════════════════════════════════════════════
   derogold-crypto.js — self-contained DeroGold address / key primitives

   Pure JavaScript. No dependencies, no network access, no WebAssembly.
   Every routine mirrors a specific function in the DeroGold daemon source, so
   ...
   Exposed as window.DeroGoldCrypto.
   ═══════════════════════════════════════════════════════════════════════════ */
...
(function (global) {
  ...
  const ADDRESS_PREFIX = 8411;

  /* src/config/WalletConfig.h

     DeroGold has one integrated address form. WalletConfig.h derives
     integratedAddressLength from a 64-character payment ID and nothing
     defines a short one, so a 16-character payment ID - which WrkzCoin and
     some other forks accept - would build an address this chain's wallets
     reject. Only the long form is offered here. */
  const STANDARD_ADDRESS_LENGTH   = 97;
  const LONG_PAYMENT_ID_LENGTH    = 64;
  const INTEGRATED_ADDRESS_LENGTH = 185;   // 97 + (64 * 11 / 8)
  ...
  /* Decodes any DeroGold address, standard or integrated, and reports which it
  ...
  function decodeAddress(address) {
    ...
    if (paymentIdLength !== 0 && paymentIdLength !== LONG_PAYMENT_ID_LENGTH) {
    ...
    return {
      ...
      paymentId,
      paymentIdType: paymentIdLength === LONG_PAYMENT_ID_LENGTH ? 'long' : null,
  ...
  function createIntegratedAddress(address, paymentId) {
    ...
    if (paymentId.length !== LONG_PAYMENT_ID_LENGTH) {
      throw new Error(
        `Payment ID must be ${LONG_PAYMENT_ID_LENGTH} `
