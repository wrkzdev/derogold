...
const COIN = {
  ...
  blockTime:   300,         // DIFFICULTY_TARGET_V3 = 300 seconds, from height 2325000
  defaultPort: 6969,
  addressPrefix: 8411,
};
...
// Default: use the /api path proxied by nginx to localhost:6969.
...
async function doSearch(query) {
  ...
  if (window.DeroGoldCrypto
      && (query.length === window.DeroGoldCrypto.STANDARD_ADDRESS_LENGTH
       || query.length === window.DeroGoldCrypto.INTEGRATED_ADDRESS_LENGTH)) {
    try {
      window.DeroGoldCrypto.decodeAddress(query);
      hideLoading();
  ...
  showError(`No results found for: <code>${escHtml(query)}</code>. `
          ...
          + `(64 hex chars), or a DeroGold address to decode.`);
}
    ...
    <div class="detail-card full mt">
      ...
      <div class="check-form">
        <div class="check-row">
          ...
          <input id="ckAddr" class="check-input" type="text" placeholder="dg… address" spellcheck="false" autocomplete="off">
        </div>
        <div class="check-row">
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   Everything below runs entirely in the browser against vendor/derogold-crypto.js.
   No key material, seed, or address is ever sent to the daemon or anywhere else.
   ═══════════════════════════════════════════════════════════════════════════ */

function deroGoldCrypto() {
  if (!window.DeroGoldCrypto) {
    throw new Error('vendor/derogold-crypto.js failed to load, so these tools are unavailable.');
  }
  return window.DeroGoldCrypto;
}
...
function randomPaymentId(length) {
  ...
  return deroGoldCrypto().hex(bytes);
}
...
function initPaperWallet() {
  ...
  btn.addEventListener('click', () => {
    ...
    try {
      const wallet = deroGoldCrypto().createWallet();
...
function initImportSeed() {
  ...
  const run = () => {
    ...
    try {
      const wallet = deroGoldCrypto().importWallet(input);
    ...
    try {
      ...
      $('intResult').innerHTML = /* html */ `
        <div class="detail-card full">
          ...
          <div class="tool-badges">
            <span class="badge badge-accent">Payment ID</span>
...
function decodeAddressForDisplay(address) {
  ...
  const kind = d.isIntegrated ? 'Integrated address' : 'Standard address';
