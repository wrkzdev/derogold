...
vm.runInContext(fs.readFileSync(path.join(ROOT, 'vendor/derogold-crypto.js'), 'utf8'), sandbox,
  { filename: 'derogold-crypto.js' });
...
const addrMatch = paper.match(/dg[1-9A-HJ-NP-Za-km-z]{95}/);
...
const PID = 'a1b2c3d4e5f60718'.repeat(4);
el('intAddr').value = LIVE;
el('intPid').value = PID;
el('intBtn').click();
const intOut = el('intResult').innerHTML;
check('integrated result rendered', intOut.length > 0);
check('marked as carrying a payment ID', intOut.includes('Payment ID'));
check('reports 185 characters', intOut.includes('185 characters'));
check('echoes the payment ID', intOut.includes(PID));
check('shows the original standard address', intOut.includes(LIVE));
check('integrated status ok', el('intStatus')._classes.has('ok'));

const intAddrMatch = intOut.match(/dg[1-9A-HJ-NP-Za-km-z]{183}/);
check('produced a 185-char integrated address', !!intAddrMatch);
...
check('shows the prefix', decOut.includes('8411'));
...
check('integrated address decodes',
  decOut.includes('Integrated address'), decOut.slice(0, 300));
check('extracts the payment ID', decOut.includes(PID));
check('extracts the base address', decOut.includes(LIVE));

el('decAddr').value = LIVE.slice(0, 96) + 'X';
el('decBtn').click();
