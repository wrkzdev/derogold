...
const cryptoTag = html.search(/<script src="vendor\/derogold-crypto\.js(\?[^"]*)?"/);
...
check('derogold-crypto.js is loaded before app.js', cryptoTag !== -1 && cryptoTag < appTag);

check('the crypto module file exists',
  fs.existsSync(path.join(ROOT, 'vendor', 'derogold-crypto.js')));
...
const CACHED_ASSETS = ['style.css', 'app.js', 'vendor/derogold-crypto.js'];
