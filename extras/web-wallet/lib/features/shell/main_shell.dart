...
class _MainShellState extends ConsumerState<MainShell> {
  ...
  /* The most notifications one transaction update may raise, so a burst
     cannot bury the page under them. */
  ...
  void _onSyncStatusChange(
    ...
    if (!autosaveOn) {
      ...
      // rather than leaving it off until the page is reloaded.
  ...
  /// Autosave matters more here than on the desktop: the module's filesystem
  /// lives in the tab's memory, so anything not written back to IndexedDB is
  /// gone the moment the tab is closed. `WalletCApi.save()` does both halves.
  ...
  void _showNotification(Transaction tx) {
    ...
    final title = tr?.degoReceived ?? 'DEGO Received';
    final body = tr?.youReceivedAmount(
            formatAmount(tx.totalAmount.abs(), showTicker: true)) ??
  ...
  Widget build(BuildContext context) {
    ...
    // Built, not read for its value: this is what drains the module's event
    // queue, and nothing else keeps it alive. Without it the wallet syncs in
    // the worker and the UI never hears about it until the next timed poll.
    ref.watch(walletEventPumpProvider);
    ...
    if (isNarrow) {
      // ── Narrow: drawer-based navigation ──────────────────────────────────
      return Scaffold(
        appBar: AppBar(
          title:
              Text(tabs[selected].label, style: const TextStyle(fontSize: 16)),
        ...
        drawer: Drawer(
          backgroundColor: surface,
          child: _buildSidebarContent(context, tabs, selected, surface, tr,
              isDrawer: true),
        ),
        body: Column(
          children: [
...
/// Lives in the shell so it is on every screen, and stays up once the wallet
/// is synced — which is exactly when a balance missing its older half looks
/// most trustworthy. See LITENODE.md.
...
class _NodeStatusFooter extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final isOnline =
        nodeInfoAsync.valueOrNull?['daemonOnline'] as bool? ?? false;
