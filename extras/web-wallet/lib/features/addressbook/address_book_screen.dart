...
// ── DEGO address validation ───────────────────────────────────────────────────
// Standard: 97 chars | Integrated (address + payment ID): 185 chars.
// All start with "dg" and consist of base58 characters.
bool _isValidDeroGoldAddress(String address) {
  const validLengths = {97, 185};
  if (!validLengths.contains(address.length)) return false;
  if (!address.startsWith('dg')) return false;
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () {
                ...
                if (!_isValidDeroGoldAddress(addr)) {
                  setState(() => error =
                      tr?.invalidDeroGoldAddress ??
                      'Invalid DeroGold address. Must be 97 characters '
                      '(standard) or 185 (integrated), starting with "dg".');
                  return;
                }
