...
class _TransferScreenState extends ConsumerState<TransferScreen> {
  ...
  void _startPowPolling() {
    _powLabel = null;
    _powTimer?.cancel();
    _powTimer = Timer.periodic(const Duration(milliseconds: 500), (_) async {
      if (!mounted || !_loading) { _stopPowPolling(); return; }
      // Nothing here is synchronous: the PoW runs in a worker and every
      // question about it is a message round trip.
      final ffi = ref.read(walletCApiProvider);
      final (:active, :elapsedMs, nonces: _) = await ffi.getPowStatus();
      if (!mounted || !_loading || !active) return;
  ...
  ///
  /// Nothing is built or sent here. The wallet backend has no way to construct
  /// a transaction without relaying it, so the review confirms what the user
  /// asked for — address, amount, payment ID — and the fee and ring size are
  /// known only once it has actually been sent.
  void _review() {
    final tr = S.of(context);
    ...
    if (!isValidDeroGoldAddress(dest)) { setState(() => _error = tr?.invalidDeroGoldAddress ?? 'Invalid DeroGold address'); return; }
  ...
  Future<void> _send() async {
    final dest = _addressCtrl.text.trim();
    final atomic = parseAmount(_amountCtrl.text.trim());
    if (dest.isEmpty || atomic == null || atomic <= 0) return;
    final paymentId = _paymentIdCtrl.text.trim();
  ...
  void _reset() {
    ...
    setState(() { _step = _TransferStep.form; _sent = null; _error = null; });
  }
                      ...
                      children: [
                        ...
                        Text(
                          _sweepMode
                              ...
                              : (tr?.sendDegoToAny ?? 'Send DEGO to any address'),
          ...
          children: [
            ...
            SizedBox(
              ...
              child: FilledButton(
                ...
                child: Text(tr?.reviewTransaction ?? 'Review Transaction'),
              ),
            ),
          ],
        ),
        ...
        child: Column(
          ...
          children: [
            ...
            _ReviewRow(
              ...
              value: '${formatAmount(amount)} $kCoinTicker',
              bold: true,
            ),
            // No fee here on purpose: it is settled when the transaction is
            // built, which is the same moment it is sent, so any figure shown
            // now would be a guess. The exact fee is on the next screen.
            _ReviewRow(
              label: tr?.fee ?? 'Fee',
              value: 'deducted when the transaction is sent',
            ),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        tr?.ringSizeReduced(s.mixin + 1, s.defaultMixin + 1) ??
                            'Ring size reduced to ${s.mixin + 1} (normally ${s.defaultMixin + 1}). '
                                'The amounts sent did not have enough outputs on the chain to '
