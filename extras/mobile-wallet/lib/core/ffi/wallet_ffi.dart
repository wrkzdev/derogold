/// wallet_ffi.dart
///
/// Dart FFI binding for the wallet_capi shared library, built from this
/// repository:
///
...
///
/// Android : libwallet_capi.so  (bundled in
///           `android/app/src/main/jniLibs/<abi>/`)
/// iOS     : linked statically or loaded via DynamicLibrary.process()
///
/// Usage:
///   final api = WalletCApi();
///   await api.create('/data/.../my.wallet', 'pass',
///       'dego-node-rpc.0z.network', 6969);
///   // ... use api ...
///   api.close();
library;
...
/// background threads at all", which leaves the wallet parked at its start
/// height forever.
///
/// Capped low: phones throttle hard and the sync threads are CPU bound.
...
// wallet_send_advanced_json (handle, requestJson, **outResult, *len).
// There is no broadcast flag: this backend builds and relays in one step.
...
// wallet_get_tx_private_key (handle, hashHex, **out, *len)
...
class WalletCApi {
  ...
  static DynamicLibrary _openLibrary() {
    if (Platform.isAndroid) return DynamicLibrary.open('libwallet_capi.so');
    if (Platform.isIOS) return DynamicLibrary.process();
    // Allow desktop for development/testing.
  ...
  /// the payload grows with the whole transaction history.
  ...
  /// hence [getPowStatus] to show progress while it runs. The transaction is
  /// relayed as part of the same call; there is no build-then-broadcast.
  ...
  /// The transaction is built and relayed in one step — this backend cannot
  /// build one without sending it — so the fee and ring size are known only
  /// after the send, never before it.
