...
class _SettingsScreenState extends ConsumerState<SettingsScreen> {
  ...
  final _customPortCtrl = TextEditingController(text: '6969');
  ...
  void _loadCurrentNode() {
    ...
    if (nodeInfo != null) {
      ...
      final port = nodeInfo['daemonPort'] as int? ?? 6969;
  ...
  ({String host, int port, bool ssl})? _formNode() {
    ...
    return (
      host: host,
      port: int.tryParse(_customPortCtrl.text) ?? 6969,
      ssl: _customSsl,
    );
  }
  ...
  ///
  /// A rescan starting below `AppConfig.slowRescanHeight` walks most of the
  /// chain and runs for hours on a phone, so it is confirmed twice: once for
  /// the rescan itself, and again for the wait it commits the user to. The CLI
  /// wallet asks at the same height (`WalletConfig::slowRescanHeight`).
  Future<void> _resetScan() async {
    ...
    if (height < AppConfig.slowRescanHeight) {
      final goAhead = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: Text(tr.slowRescanTitle),
          content: Text(tr.slowRescanWarning(AppConfig.slowRescanHeight),
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () => Navigator.pop(ctx, true),
              child: Text(tr.slowRescanContinue),
            ),
          ],
        ),
      );
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                    ...
                    children: [
                      Expanded(
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                              hintText: '6969', labelText: tr.port),
                        ),
                      ),
                ...
                // What the node actually holds. A pruned or lite node answers
                // every scan from its own floor, so how far back it goes is
              ...
              ListTile(
                ...
                subtitle: Column(
                  ...
                  children: [
                    ...
                    // the sync has quietly found nothing down there.
                    if (status != null && status.isLiteNode) ...[
                      const SizedBox(height: 6),
                      Text(
                        tr.liteNodeServesFrom(status.pruneFloor),
                        style: Theme.of(context)
                            .textTheme
                            .bodySmall
          ...
          child: Column(
            children: [
              ListTile(
                ...
                subtitle: Text(tr.versionInfo(AppConfig.appVersion)),
              ),
              const Divider(height: 1, indent: 56),
              // Proper nouns: the same in every language, so no l10n keys.
              _linkTile(Icons.code, 'GitHub', AppConfig.githubUrl),
              const Divider(height: 1, indent: 56),
              _linkTile(Icons.language, 'Website', AppConfig.websiteUrl),
              const Divider(height: 1, indent: 56),
              _linkTile(
                  Icons.cloud_outlined, 'Bluesky', AppConfig.blueskyUrl),
              const Divider(height: 1, indent: 56),
              _linkTile(Icons.alternate_email, 'X', AppConfig.twitterUrl),
              const Divider(height: 1, indent: 56),
              _linkTile(Icons.forum_outlined, 'Reddit', AppConfig.redditUrl),
              const Divider(height: 1, indent: 56),
              _linkTile(Icons.article_outlined, 'Medium', AppConfig.mediumUrl),
              const Divider(height: 1, indent: 56),
              ListTile(
                leading: const Icon(Icons.gavel_outlined),
                title: Text(tr.license),
                subtitle: Text(tr.licenseText),
              ),
            ],
          ),
        ),
  ...
  /// A project link. There is no browser launcher in this app's dependency
  /// set, so tapping copies the URL rather than pretending it can open it.
  Widget _linkTile(IconData icon, String label, String url) {
    return ListTile(
      ...
      trailing: CopyButton(text: url, size: 18),
    );
  }
