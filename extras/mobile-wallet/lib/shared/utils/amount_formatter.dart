...
/// 100 atomic = 1.00 DEGO (2 decimal places).
...
/// Cheap client-side sanity check on a DeroGold address.
///
...
bool isValidDeroGoldAddress(String address) {
...
///
/// Accepts a plain address, or a `dego:<address>?amount=…&paymentId=…` URI.
...
({String address, String? amount, String? paymentId}) parseAddressPayload(
  ...
  if (!value.toLowerCase().startsWith('dego:')) {
    return (address: value, amount: null, paymentId: null);
  }
  // Uri.parse treats everything after "dego:" as an opaque path, so split by
  // hand rather than relying on host/path parsing.
  final body = value.substring('dego:'.length);
